...
- **Expo SDK 54+**: You don't need it — Expo now uses React Native's built-in `edgeToEdgeEnabled` gradle property
- **Expo SDK 53**: Still recommended for edge-to-edge support on Android
- **Bare React Native**: Optional — you can set `edgeToEdgeEnabled=true` in `gradle.properties` instead, or install `react-native-edge-to-edge` if you use libraries that detect it for ecosystem compatibility (e.g. `react-native-bootsplash`, `react-native-permissions`)
