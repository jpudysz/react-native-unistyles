...
With Expo SDK 54, Expo transitioned away from `react-native-edge-to-edge` in favor of React Native’s built-in `edgeToEdgeEnabled` `gradle.properties` flag. Because of this, **Expo SDK 54+ users don’t need to install it**.

**Bare React Native** users can also enable edge-to-edge via `gradle.properties`:
