...
**Expo SDK 54+** users don't need it — Expo now uses React Native's built-in `edgeToEdgeEnabled` gradle property.
**Bare React Native** users can set `edgeToEdgeEnabled=true` in `gradle.properties` or optionally install the package for ecosystem compatibility.
