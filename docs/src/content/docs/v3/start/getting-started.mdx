...
We put a lot of effort into making Unistyles as easy to use as possible. You no longer need the `useStyle` hook or wrap your app in React Provider. Unistyles integrates seamlessly with your existing code, so you can start using it immediately.
...
<Aside>
Check Babel plugin extra configuration options [here](/v3/other/babel-plugin#extra-configuration).
...
<Tabs>
  ...
  <TabItem label="SSR">
    ...
    <Aside type="caution" title="Babel only">
      You need to disabled SWC and rely on Babel for transpiling your code.
    </Aside>
  </TabItem>
</Tabs>
