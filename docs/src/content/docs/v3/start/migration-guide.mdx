...
2. Replace your configuration with [new](/v3/start/configuration) one.
   ...
   +             initialTheme: 'dark', // works exactly the same like in 2.0
                 ...
                 // experimentalCSSMediaQueries: this options is also removed, and enabled by default with custom parser
...
13. Some `UnistylesRuntime` methods have been removed:
    ...
    - UnistylesRuntime.statusBar.setColor(color) // remove due to Android 15 deprecation
    - UnistylesRuntime.navigationBar.setColor(color) // remove due to Android 15 deprecation
    ```
