...
Technically, yes - the minimum supported version is `0.75.0`. However, for Android, you must modify Unistyles' `CMakeLists.txt` to include all required headers.
Versions lower than `0.75.0` are not supported, as Unistyles depends on `Nitro Modules` and the latest React Native APIs. We have no plans to support older versions.
