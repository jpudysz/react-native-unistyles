...
`StyleSheet.create` should be invoked as soon as possible, before the metro bundler references any `StyleSheet` from your library.

You can then call `StyleSheet.create` multiple times to override configurations.
However, keep in mind that `StyleSheet.create` makes a roundtrip to C++, which can add a few `ms` to your app's startup time.
