...
<Aside type="note">
If you use monorepos, or project with Expo Router most likely you have multiple root folders.

In that case:
- set `root` to folder with your components eg. `@myapp/ui` or `components`
- use `autoProcessImports` config to whitelist your folder (check example below)
</Aside>

### `autoProcessImports`

This configuration should be used when you want process files containing specific imports.
...
const unistylesPluginOptions = {
    ...
    // additonaly process components from this `node_modules` package
