...
import { StyleSheet, type StyleSheetValues } from 'react-native-unistyles';

type ExampleProps = {
    maxWidth?: StyleSheetValues['maxWidth'];
    isOdd?: boolean;
};

export const Example = ({ maxWidth, isOdd, children }: PropsWithChildren<ExampleProps>) => {
    return (
...
const styles = StyleSheet.create(theme => ({
    container: ({ maxWidth, isOdd }: ExampleProps) => ({
        backgroundColor: theme.colors.background,
        flex: 1,
        justifyContent: 'center',
        alignItems: 'center',
