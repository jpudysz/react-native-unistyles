...
<Aside>
Dive deeper into the `mq` utility and its helpers [here](/v3/references/breakpoints#mq).
</Aside>
...
Notice how we replaced the JavaScript-driven opacity logic with `_hover` and `_active` selectors inside the `_web` block.
...
<CompareChanges
  commitUrl="https://github.com/jpudysz/react-native-unistyles/commit/v3w4x5y6z7a8901"
/>
