...
In this tutorial, we'll build a simple, cross-platform app that targets `iOS`, `Android`, and `Web`.
...
Both `Unistyles` and `Reanimated` require babel plugin to work.  Since a `babel.config.js` file isn't created by default, we can generate it by running:
...
**Configuration**: Finally, we called `StyleSheet.configure` function to set all the options. We also enabled `adaptiveThemes`, which will cause Unistyles to automatically transition between themes based on device color scheme.
