...
```tsx title="components/SettingTile.tsx"
...
- **`rt` (mini runtime)**: is only available inside `StyleSheet.create()` function. It contains device metadata like insets, screen dimensions, and color scheme that are relevant for styling.

- **`UnistylesRuntime`**: A global object accessible **anywhere** in your app, not just in stylesheets. It contains all the same information as `rt` plus additional methods and properties.
...
```tsx
// This will not re-render when theme changes
