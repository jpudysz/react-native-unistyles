...
When creating a StyleSheet, you can pass a function that receives both the `theme` and the `rt` as arguments.
The `rt` object contains useful device metadata, including `rt.colorScheme`.
...
Instead of specifying `type` prop manually, we can use `UnistylesVariants` genric type:
...
Our component is clean, but we can make it even better! Why are we passing `lightColor` and `darkColor` as props when Unistyles already has access to our full theme?
...
We no longer pass `lightColor` or `darkColor` as props because those colours now come straight from the theme (typography color). When you change the `colorScheme` or update the theme, Unistyles automatically injects the new values into your `StyleSheet`, so there’s nothing to manage manually. Keeping all theming logic inside the `StyleSheet` avoids duplicated work and makes the code easier to maintain.
...
Now for the `ThemedView` component. This one is much simpler.
...
<CompareChanges 
  commitUrl="https://github.com/jpudysz/react-native-unistyles/commit/m3n4o5p6q7r8901"
/>
...
<TutorialNavigation
    ...
    next={{
        title: "Part 4: Variants and Compound variants",
        href: "/v3/tutorial/variants"
    }}
/>
