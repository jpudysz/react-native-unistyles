...
Notice how much cleaner the component is! We simply pass the type prop to the useStyles hook, and Unistyles applies the correct styles from our variants block.
...
Instead of specifying `type` prop, we can use `UnistylesVariants` genric type:
