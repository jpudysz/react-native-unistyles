...
Variants, the most requested feature in version 1.0, are now available to use with other Unistyles features like media queries and breakpoints!
...
First, you need to name your group and then define variants within it:

```tsx /colors/ /sizes/ /otherGroupName/
const stylesheet = createStyleSheet(theme => ({
    container: {
        flex: 1,
        variants: {
...
```tsx /undefined/ /{}/
const styles = useStyles(stylesheet, undefined) // will use default variant
const styles = useStyles(stylesheet, {}) // will use default variant
```
...
```tsx /undefined/
const styles = useStyles(stylesheet, {
    color: undefined // will use default variant
})
```
